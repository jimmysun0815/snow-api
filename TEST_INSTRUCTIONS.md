...
### 已完成的优化:
1. ✅ **多线程并发**: 默认 10 个线程同时采集（平衡速度和稳定性）
2. ✅ **优化延迟**: OnTheSnow/MtnPowder 使用 0.5-1 秒短延迟，Open-Meteo 无延迟
3. ✅ **Open-Meteo API Key**: 使用付费 API，无速率限制
4. ✅ **进度显示**: 实时显示采集进度
5. ✅ **线程安全数据库**: 使用 scoped_session 避免并发冲突

### 预期性能:
- **串行采集** (旧方法): ~每个雪场 4-5 秒 = 300 个雪场需要 20-25 分钟
- **并发采集** (新方法): ~每个雪场 0.5-1 秒 = 300 个雪场需要 2-5 分钟
