...
本文件可独立运行，不依赖项目内 models / config 等模块。
通过跳板机访问 RDS 时使用系统 ssh（无需 sshtunnel/paramiko），请确保本机已安装 ssh 客户端。
"""
...
JUMP_PKEY_PATH = "/Users/jimmysun/Desktop/workspace/resort-data/key/snowapp-jump-server.pem"         # 跳板机 SSH 私钥文件路径（推荐）；留空则使用下面的密码
JUMP_PASSWORD = ""          # 跳板机 SSH 密码（当 JUMP_PKEY_PATH 为空时可填）

# ---------- RDS 数据库信息 ----------
RDS_HOST = "resort-data-postgres.c96os4eq6dgq.us-west-2.rds.amazonaws.com"               # RDS 内网 IP 或域名（从跳板机可访问的地址）
RDS_PORT = 5432             # RDS 端口（PostgreSQL 一般为 5432）
RDS_USER = "app"               # 数据库用户名
RDS_PASSWORD = "dCti++8PbdW6kdWW"           # 数据库密码
RDS_DB = "snow"             # 数据库名

# ---------- 连接方式 ----------
# True：通过跳板机建立 SSH 隧道再连 RDS（需上面跳板机 + RDS 信息都填好）
# False：直连数据库（RDS 若在公网或本机，填好 RDS_* 即可；否则可依赖下方环境变量）
USE_JUMP = True

# 直连时是否优先使用本配置（USE_JUMP=False 时）：若为 True 且 RDS_HOST 非空，用上面 RDS_*；否则用环境变量 DATABASE_URL / POSTGRES_*
USE_RDS_DIRECT = True
...
def _get_database_url():
    """获取直连数据库 URL：优先环境变量 DATABASE_URL，否则用 RDS_* 或 POSTGRES_*。"""
    url = os.environ.get("DATABASE_URL")
    if url:
        return url
    if USE_RDS_DIRECT and RDS_HOST and RDS_USER:
    ...
    db = os.environ.get("POSTGRES_DB", "snow")
...
def _get_lantinghei_bold_path():
    """返回 Lantinghei 加粗/Demibold 字体路径，用于标题。优先项目内 fonts 目录。"""
    for d in (os.path.join(_backend_dir, "daily_report", "fonts"), os.path.join(_backend_dir, "fonts")):
        if os.path.isdir(d):
...
def _get_chinese_font():
    """
    返回支持中文的字体路径，供 Pillow 使用。
    优先：环境变量、项目内 fonts 目录、Lantinghei SC、PingFang 等。
    """
    path = os.environ.get("SNOWFALL_IMAGE_FONT_PATH")
    if path and os.path.isfile(path):
        return path
    # 优先使用 daily_report 目录下的 Lantinghei SC.ttf（与 report_bg 同目录）
    daily_report_font = os.path.join(_backend_dir, "daily_report", "Lantinghei SC.ttf")
    if os.path.isfile(daily_report_font):
        return daily_report_font
    # 其他项目内字体：daily_report/fonts、backend-api/fonts
    local_dirs = [
        os.path.join(_backend_dir, "daily_report", "fonts"),
        os.path.join(_backend_dir, "fonts"),
    ]
    for d in local_dirs:
        if os.path.isdir(d):
            for name in os.listdir(d):
...
def _get_logo_path():
    """
    返回 logo 图片路径。优先环境变量 SNOWFALL_IMAGE_LOGO_PATH，
    否则使用项目根下 marketing/logo/逐风定稿-01.png。
    """
    path = os.environ.get("SNOWFALL_IMAGE_LOGO_PATH")
    if path and os.path.isfile(path):
        return path
    # 项目根：backend-api 的上一级
    project_root = os.path.dirname(_backend_dir)
    # sample 使用逐风定稿-02 作为 logo（与 report-sample.svg 一致）
    for name in ("逐风定稿-02.png", "逐风定稿-02.jpg", "逐风定稿-01.png", "逐风定稿-01.jpg"):
        p = os.path.join(project_root, "marketing", "logo", name)
        if os.path.isfile(p):
            return p
    return None
...
def _get_sample_svg_path():
    """report-sample.svg 路径（与 sample 完全一致时使用）。"""
    project_root = os.path.dirname(_backend_dir)
    return os.path.join(project_root, "marketing", "ios", "report-sample.svg")


def _get_report_bg_path(output_dir=None):
    """report_bg.png 路径，优先环境变量 SNOWFALL_REPORT_BG，否则用 output_dir 或 daily_report 下的 report_bg.png。"""
    path = os.environ.get("SNOWFALL_REPORT_BG")
    if path and os.path.isfile(path):
        return path
    base = (output_dir or DEFAULT_OUTPUT_DIR) if output_dir is not None else os.path.join(_backend_dir, "daily_report")
...
# 默认图片输出目录：backend-api/daily_report
DEFAULT_OUTPUT_DIR = os.path.join(_backend_dir, "daily_report")
